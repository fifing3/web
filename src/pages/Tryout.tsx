...
import { Search, ChevronRight, Clock, FileText, Award, BarChart, CheckCircle2, ShieldCheck } from 'lucide-react';
...
export default function Tryout() {
  ...
  return (
    ...
    <div className="flex flex-col w-full pb-20">
      ...
      {/* Header */}
      <section className="bg-slate-900 pt-16 pb-24 px-4 text-center relative overflow-hidden">
        <div className="absolute inset-0 bg-blue-900/20 mix-blend-overlay pointer-events-none"></div>
        <div className="max-w-3xl mx-auto relative z-10">
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight">Paket Tryout</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredPackages.map((pkg, idx) => (
              <div key={idx} className="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col group">
                <div className="h-32 bg-gradient-to-r from-blue-600 to-blue-800 relative p-6 flex flex-col justify-end text-white">
                  {pkg.type === 'Gratis' ? (
                     <div className="absolute top-4 right-4 bg-emerald-500/90 backdrop-blur text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                       GRATIS
                     </div>
                  ) : (
                     <div className="absolute top-4 right-4 bg-amber-500/90 backdrop-blur text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                       PREMIUM
                     </div>
                  )}
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h3 className="font-bold text-lg text-slate-900 mb-2 leading-tight line-clamp-2">{pkg.title}</h3>
                  ...
                  <div className="space-y-2 mb-6 flex-grow">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <CheckCircle2 size={16} className="text-emerald-500" /> Nilai Otomatis
                    </div>
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <CheckCircle2 size={16} className="text-emerald-500" /> Pembahasan Lengkap
                    </div>
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <CheckCircle2 size={16} className="text-emerald-500" /> Ranking Nasional
                    </div>
                  </div>
